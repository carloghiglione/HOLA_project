...
#  Fixed parameters:
#   •dirichlet parameters and cauchy parameters -> determining parameters of the actual dirichlet parameters
#       -> it generates a random, possibly wild, parametrization over the dirichlet of the hyperparameters
#           -> once the environment is initialized, the parameters of the dirichlet stay fixed, they are just unknown
#   •cauchy parameters for poisson parameters -> determining the parameters of the actual poissons
#       -> it generates a random, possibly wild, mean for the poisson of the hyperparameters
#           -> once the environment is initialized, such mean for the population stay fixed, they are just unknown

class Hyperparameters:
    def __init__(self, transition_prob_listofmatrix, dir_hyperparams_listofvector, dir_cauchy_vector, pois_cauchy_vector, conversion_rate_listofmatrix, margin_matrix):
        self.global_transition_prob = transition_prob_listofmatrix
        self.dir_params = self.sample_dirichlet_params(dir_hyperparams_listofvector, dir_cauchy_vector)
        self.pois_param = self.sample_poisson(pois_cauchy_vector)
        self.global_conversion_rate = conversion_rate_listofmatrix
        self.global_margin = margin_matrix

    def sample_dirichlet_params(self, dir, cau):
        ret = []
        for i in range(3):
            base_distribution = np.array(npr.dirichlet(alpha=dir[i], size=1)[0],dtype=float)
            scale_param = np.absolute(cau[i] + npr.standard_cauchy(size=1))
            base_distribution = scale_param*base_distribution
            ret.append(copy.deepcopy(base_distribution))
        return ret

    def sample_poisson(self, cauchy):
        pois = np.zeros(shape=3, dtype=float)
        for j in range(3):
            pois[j] = np.absolute(npr.standard_cauchy(size=1) + cauchy[j])
        return pois
