import numpy as np
from P1_Base.Price_puller import greedy_pull_price_exact
...
class Items_TS_Learner:
    ...
    def pull_prices(self, env: Hyperparameters):
        conv_rate = -1*np.ones(shape=(5, 4))
        for i in range(5):
            conv_rate[i, :] = self.learners[i].pull_cr()
        prices = greedy_pull_price_exact(env=env, conv_rates=conv_rate, alpha=env.dir_params, n_buy=env.mepp,
                                         trans_prob=env.global_transition_prob)
        return prices
