from Classes import *
from TS_trial import Items_TS_Learner
import numpy as np
...
for i in range(5):
    best_prices[i] = np.argmax(env.global_margin[i,:]*(env.global_conversion_rate[0][i,:]*env.pois_param[0] + env.global_conversion_rate[1][i,:]*env.pois_param[1] + env.global_conversion_rate[2][i,:]*env.pois_param[2]))
