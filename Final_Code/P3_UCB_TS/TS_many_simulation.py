...
for sim in range(n_trials):
    ...
    print("=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*")
    sys.stdout.write('\r' + str("Simulation n.") + str(sim+1) + str(" out of ") + str(n_trials+1) + '\n')
...
plt.fill_between(range(time_horizon), mean_prof - sd_prof, mean_prof + sd_prof, alpha=0.4, color='blue')
plt.fill_between(range(time_horizon), mean_prof_cl - sd_prof_cl, mean_prof_cl + sd_prof_cl, alpha=0.4, color='red')
...
plt.figure(1)
plt.plot(mean_reg, color='blue')
plt.plot(mean_psereg, color='red')
plt.fill_between(range(time_horizon), mean_reg - sd_reg, mean_reg + sd_reg, alpha=0.4, color='blue')
plt.fill_between(range(time_horizon), mean_psereg - sd_psereg, mean_psereg + sd_psereg, alpha=0.4, color='red')
plt.legend(["Regret", "Pseudoregret"], loc='best')
...
for i in range(n_trials):
    plt.plot(profits[i, :], color='red')
    plt.plot(profits_cl[i, :], color='blue')
plt.legend(["TS", "Optimal"], loc='best')
plt.title("Profits realizations")
plt.xlabel("time [day]")
plt.ylabel("profit [euros]")
plt.tight_layout()
plt.show()
...
plt.figure(3)
for i in range(n_trials):
    plt.plot(regret[i, :], color='red')
    plt.plot(pseudoregret[i, :], color='blue')
plt.legend(["TS", "Optimal"], loc='best')
plt.title("Regret realizations")
plt.xlabel("time [day]")
plt.ylabel("regret [euros]")
plt.tight_layout()
plt.show()
