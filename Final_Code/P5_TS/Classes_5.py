...
class User:
    def __init__(self, website: Daily_Website, starting_product, u_type):
        ...
        self.show = np.zeros(shape=(5, 5))
        self.click = np.zeros(shape=(5, 5))
        self.trans_order = website.trans_order
...
class Day:
    def __init__(self, g_web: Hyperparameters, prices):
        ...
        self.shows = np.zeros(shape=(5, 5))
        self.cliks = np.zeros(shape=(5, 5))
        ...
        for t in range(3):
            for p in range(5):
                for j in range(self.n_users[t, p]):
                    ...
                    self.shows += user.show
                    self.cliks += user.click
