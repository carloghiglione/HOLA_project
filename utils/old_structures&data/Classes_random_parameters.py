...
class Hyperparameters:
    def __init__(self, transition_prob_listofmatrix, dir_hyperparams_listofvector, dir_pois_vector, pois_pois_vector, conversion_rate_listofmatrix, margin_matrix):
        self.global_transition_prob = transition_prob_listofmatrix
        self.dir_params = self.sample_dirichlet_params(dir_hyperparams_listofvector, dir_pois_vector)
        self.pois_param = self.sample_poisson(pois_pois_vector)
        self.global_conversion_rate = conversion_rate_listofmatrix
        self.global_margin = margin_matrix

    def sample_dirichlet_params(self, dir, poi):
        ret = []
        for i in range(3):
            base_distribution = np.array(npr.dirichlet(alpha=dir[i], size=1)[0],dtype=float)
            scale_param = npr.poisson(lam=poi[i], size=1)
            base_distribution = scale_param*base_distribution
            ret.append(copy.deepcopy(base_distribution))
        return ret

    def sample_poisson(self, poi):
        pois = np.zeros(shape=3, dtype=float)
        for j in range(3):
            pois[j] = npr.poisson(poi[j], size=1)
        return pois
